...
"""
    ...
    for container in containers:
        ...
        # 容器 {container_id} 的文件上传路径
        location /{container_id}/upload {{
            proxy_pass http://127.0.0.1:{download_port}/upload;
            proxy_set_header Host $host;
            proxy_set_header X-Real-IP $remote_addr;
            proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
            proxy_set_header X-Forwarded-Proto $scheme;
...
"""
        ...
        location /{container_id}/notebook/ {{
            ...
            # 替换HTML内容中的相对路径
            sub_filter_once off;
            sub_filter 'src="/notebook/' 'src="/{container_id}/notebook/';
            sub_filter 'href="/notebook/' 'href="/{container_id}/notebook/';
            sub_filter '"/notebook/' '"/{container_id}/notebook/';
...
def write_nginx_config(config, filepath='./nginx.conf'):
    """写入Nginx配置文件"""
    try:
...
def reload_nginx():
    """重新加载Nginx配置"""
    try:
        # 使用docker exec重新加载nginx容器
        result = subprocess.run(['docker', 'exec', 'python-mcp-notebook-nginx-proxy-1', 'nginx', '-s', 'reload'], 
        ...
        if result.returncode == 0:
            ...
            # 如果reload失败，尝试重新启动容器
            restart_result = subprocess.run(['docker', 'compose', '-f', 'docker-compose.proxy.yml', 'restart'], 
                                          capture_output=True, text=True)
            if restart_result.returncode == 0:
                print("Nginx容器重启成功")
                return True
            else:
                print(f"Nginx容器重启失败: {restart_result.stderr}")
            return False
    except Exception as e:
...
def start_nginx_container():
    """启动Nginx代理容器"""
    try:
        # 检查nginx-proxy容器是否已经运行
        result = subprocess.run(['docker', 'ps', '-q', '-f', 'name=python-mcp-notebook-nginx-proxy-1'], 
                              capture_output=True, text=True)
        if not result.stdout.strip():
            # 启动nginx-proxy容器
            print("启动Nginx代理容器...")
            subprocess.run(['docker', 'compose', '-f', 'docker-compose.proxy.yml', 'up', '-d'])
            time.sleep(5)  # 等待容器启动
        else:
            print("Nginx代理容器已在运行")
        return True
    except Exception as e:
        print(f"启动Nginx容器失败: {e}")
        return False
